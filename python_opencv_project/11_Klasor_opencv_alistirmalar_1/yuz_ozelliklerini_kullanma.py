...
def findMaxContours(contours):#max alanı bulmak için fonksiyon
    ...
    for i in range(len(contours)):
        area_face=cv2.contourArea(contours[i])#bütün contourların alanını bulucak
...
while 1:
    ...
    if len(contours)>0:#eğer contour 0 dan küçükse yani küçük siyahlıklarsa
            ...
            extBot=tuple(c[c[:,:,1].argmax()][0])#y max noktayı buldu
            ...
            cv2.line(roi,extRight,extBot,(255,255,0),2)
            cv2.line(roi,extBot,extLeft,(255,255,0),2)
