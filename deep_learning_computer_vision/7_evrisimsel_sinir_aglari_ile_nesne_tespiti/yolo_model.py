...
class YOLO:
    def __init__(self, obj_threshold, nms_threshold):
        """YOLOv3 model yüklemesi ve eşik ayarları"""
        self._t1 = obj_threshold
        self._t2 = nms_threshold
        self._yolo = load_model('data/yolo.h5', compile=False)

    def _process_feats(self, out, anchors, mask):
        """YOLO çıktı özelliklerini işleme"""
        grid_h, grid_w, num_boxes = map(int, out.shape[1:4])
        anchors = [anchors[i] for i in mask]
        anchors_tensor = tf.reshape(tf.convert_to_tensor(anchors, dtype=tf.float32), (1, 1, 1, -1, 2))
        
        out = out[0]  # batch size = 1 kabul ediliyor
    ...
    def _filter_boxes(self, boxes, box_confidences, box_class_probs):
        """Eşik değerine göre kutu filtreleme"""
    ...
    def _yolo_out(self, outs, shape):
        """YOLO modelinin çıktısını kutulara, sınıflara ve skorlarına dönüştür"""
        ...
        # Giriş görüntü boyutuna geri ölçekle
